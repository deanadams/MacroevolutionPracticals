...
First recall why it is necessary to consider the phylogenetic relationships among taxa when performing comparative analyses. In short, because species share an evolutionary history, more closely related species are expected to be similar in their trait values due solely to evolutinary descent. Thus, they cannot be treated as independent observations (they are evolutionarily correlated with one another). This can cause downstream challenges with statistical inference of macroevolutionary patterns. 
...
You can click each of the buttons to see how to perform phylogenetic regression under each method.  Notice that PIC, PGLS, and Phylo-Transform all yield equivalent results!
